...
import { InFileRepository } from "./adapter/secondary/in-file-repository";
import { FieldJsonMapper, UnitJsonMapper, GameJsonMapper, PlayerJsonMapper } from "./adapter/secondary/json-mappers";
...
iocContainer.bind<Repository<Field>>(TYPES.FIELD_REPOSITORY)
    .toConstantValue(new InFileRepository<Field>(new FieldJsonMapper(), "fields"));
iocContainer.bind<Repository<Game>>(TYPES.GAME_REPOSITORY)
    .toConstantValue(new InFileRepository<Game>(new GameJsonMapper(), "games"));
iocContainer.bind<Repository<Player>>(TYPES.PLAYER_REPOSITORY)
    .toConstantValue(new InFileRepository<Player>(new PlayerJsonMapper(), "players"));
iocContainer.bind<Repository<Unit>>(TYPES.UNIT_REPOSITORY)
    .toConstantValue(new InFileRepository<Unit>(new UnitJsonMapper(), "units"));
