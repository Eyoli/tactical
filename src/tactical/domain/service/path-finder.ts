...
class PathFinder<N> {
	...
	withNeighbourFilter(filter: EdgeFilter<N>): PathFinder<N> {
...
export class PathFinderManager<N> {
	...
	getShortestPathFromPosition<P>(graph: Graph<N>, getClosestNode: PositionMapping<P, N>, start: P, end: P): PathFinder<N> {
		const startNode = getClosestNode(start);
		const endNode = getClosestNode(end);
		return new PathFinder(graph, startNode, endNode);
	}

	getShortestPath(graph: Graph<N>, startNode: N, endNode: N): PathFinder<N> {
		return new PathFinder(graph, startNode, endNode);
	}
